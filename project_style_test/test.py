...
async def main():
    ...
    for CHAT in CHATS:
        ...
        cs = None
        if texts: texts, ad, cs = drop_channel_sign(texts)
        print(cs)
        chat_df = pd.DataFrame({'channel': chats, 'id': ids, 'date': dates, 'text': texts, 'ad': ad})
...
async def channel_info(pc):
    ...
    for chat in pc:
        ...
        try:
            ...
            CHANNELS[chat[0]]["image_url"] = entity.photo
...
for i in cs:
    print(cs[i]['title'], '\t', cs[i]['description'], '\t', cs[i]['image_url'])